...
function checkGlobalConfig() {
  ...
  const globalConfigDirPath = path.join(userHome, '.warbler-cli');
  // 检查是否存在 .warbler-cli 目录, 不存在就创建一个
  fse.ensureDirSync(globalConfigDirPath);
  // 获取全局配置文件 warbler.json 所在的路径
  const globalConfigPath = path.join(globalConfigDirPath, 'warbler.json');
...
function checkUserConfig() {
  ...
  const userConfigPath = path.join(userConfigDirPath, 'warbler.json');
